...
'bonus', 'total_stock_value', 'expenses', 'loan_advances', 'exercised_stock_options', 'long_term_incentive', 
'restricted_stock']
#Removed: 'director_fees', 'other', 'restricted_stock_deferred'
...
#Create new features - emails to/from pois as share of total emails 
...
#Select the k-best features for training set
...
sv_clf = svm.SVC(kernel='linear', C=100, random_state=42)
nv_clf = naive_bayes.GaussianNB()
dt_clf = tree.DecisionTreeClassifier(criterion='entropy', random_state=42, min_samples_split=5)
ad_clf = ensemble.AdaBoostClassifier(learning_rate=5., random_state=42)
rf_clf = ensemble.RandomForestClassifier(criterion='entropy')
kn_clf = KNeighborsClassifier(n_neighbors = 3)

classifiers = [sv_clf, nv_clf, dt_clf, ad_clf, rf_clf, kn_clf]
